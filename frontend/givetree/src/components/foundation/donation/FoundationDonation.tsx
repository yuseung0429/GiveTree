...
import donateFoundation from '@/actions/donation/donateFoundation';
...
export default function FoundationDonation({
  ...
  const [state, action, isPending] = useActionState(donateFoundation, {});

  useEffect(() => {
    if (state.success) {
      (async () => {
        await alert('재단 일시후원이 완료되었습니다.');
